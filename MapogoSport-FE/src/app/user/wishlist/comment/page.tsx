...
const CommentPage = () => {
    ...
    const [reviews, setReviews] = useState<(FieldReviewUser | ProductReview)[]>([]);
    const [filteredReviews, setFilteredRiviews] = useState<(FieldReviewUser | ProductReview)[]>([]);
    ...
    const { data, error, isLoading } = useSWR<(FieldReviewUser | ProductReview)[]>(apiFilter ? apiFilter : null, fetcher, {
    ...
    useEffect(() => {
        if (data) {
            const sortedData = data.sort((a, b) => new Date(b.datedAt).getTime() - new Date(a.datedAt).getTime());
            ...
            if (username) {
                ...
                fetch(url, {
                    ...
                    setFilteredRiviews((prev) => prev.filter((review) => isFieldReview
                        ? (review as FieldReviewUser).fieldReviewId !== id
    ...
    if (error) return <UserLayout>
        ...
        <div>Đã xảy ra lỗi trong quá trình lấy dữ liệu! Vui lòng thử lại sau hoặc liên hệ với quản trị viên</div>
    </UserLayout>;
            ...
            <UserLayout>
                ...
                {filteredReviews && filteredReviews.length > 0 ? (
                    filteredReviews.map((review) => {
                        let key = '';
                        if (selectedOption === '1') {
                            const fieldReview = review as FieldReviewUser;
                            ...
                            return (
                                <div className="box-comment-container mb-2" key={key}>
                                    <div className="d-flex justify-content-between align-items-center">
                                        ...
                                        <Link href={`/categories/sport_field/detail/${fieldReview.sportFieldId}`}
                                            className="box-comment" style={{ fontSize: '15px' }}>
                                            <b>{fieldReview.fullname}</b> đã đánh giá sân <b>{fieldReview.sportFieldName}</b>.
