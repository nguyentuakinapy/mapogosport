...
//       //  {chatListRealTime.filter((itemMessage)=>(itemMessage.receiver.username === currentUser?.username && itemMessage.sender.username === receiver)
//                   // || (itemMessage.receiver.username === receiver && itemMessage.sender.username === currentUser?.username) ).map((msg, index) => (
...
//       if( (receivedMessage.receiver === currentUser?.username && receivedMessage.sender === chat?.username)
//         || (receivedMessage.receiver === chat?.username && receivedMessage.sender === currentUser?.username)) {
...
//   console.log('chatListRealTime==>>>>>>', chatListRealTime);
...
//         return {...item,topic: `${sortedUsers[0]}-${sortedUsers[1]}`,
...
//                       (
//                       (itemMessage.receiver.username||itemMessage.tempReceiver === currentUser?.username
//                         && itemMessage.sender.username||itemMessage.tempSender === receiver)
//                     ||
//                     (itemMessage.receiver.username||itemMessage.tempReceiver === receiver && itemMessage.sender.username||itemMessage.tempSender === currentUser?.username)
//                   )
          ...
          {showChatList && (
            ...
            >
              ...
              <div className="card-body overflow-auto">
                ...
                <ListGroup>
                  {/* {selectedChat && chatListCurrentUserByDMM.length > 0 &&
                    !filteredChatList.find(item => item.receiver.username === selectedChat.username) ?(
                      <>ok</>
                    ) : (
                      <ListGroup.Item
                        onClick={() => handleChatToggle()}
                      ...
                      >
                        <div className="d-flex align-items-center">
                          <img
                            ...
                            alt={selectedChat && selectedChat.username || "Không có tên"}
                            style={{
                              width: "40px",
                              height: "40px",
                              borderRadius: "50%",
                          ...
                          />
                          <strong>
                            {selectedChat && selectedChat.fullname || "Không có tên"}
                          </strong>
                        </div>
                        <div>
                          <p className="mb-1 r-100">Chat với chủ sân</p>
                        </div>
                      </ListGroup.Item>
                    )
                  } */}
