...
import {Button,Col,Form,Image,InputGroup,Modal,Nav,OverlayTrigger,Row,Table,Tooltip,
...
const ProductAddNew = ({
  ...
  const option = [
    ...
    const [selectedProductDetail, setSelectedProductDetail] = useState<ProductDetail>([]); // lấy sản phẩm chi tiết chi tiết theo ID product
    // Fetch ProductDetail dựa vào productId

  // Chỉ fetch dữ liệu khi currentProduct?.productId có giá trị
  const { data: productDetails, error: errorProductDetails } = useSWR<ProductDetail[]>(
    currentProduct?.productId ? `${BASE_URL}/rest/product-detail/${currentProduct?.productId}` : null,
    fetcher
  );
...
const [imageProductDetail, setImageProductDetail] = useState<File | string>("");
  ...
  const handleSave = async (
    ...
    try {
      ...
      for (let [key, value] of formData.entries()) {
        console.log(
          `${index++} Key: ${key}, Type of: ${typeof value}, Value:`,
          value
        );
      }
  ...
  const deleteProductDetailSize = async (productDetailSizeId: number) => {
    ...
    try {
        const response = await axios.delete(`http://localhost:8080/rest/product-detail-size/delete/${productDetailSizeId}`);
...
const handleUpdateProductDetail = async (productDetailId: number) => {
  ...
  try {
    ...
    // Gọi API
    await axios.put(`${BASE_URL}/rest/product-detail/update/${productDetailId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
...
const handleDeleteProductDetail = async (currentProductDetailID : number) => {
  ...
  try {
    await axios.delete(`${BASE_URL}/rest/product-detail/delete/${currentProductDetailID}`);
...
const handleDeleteProductDetailGallery = async (currentProductDetailGalleryID : number) => {
  const confirmed = window.confirm("Bạn có chắc chắn muốn xóa sản phẩm này?");
  ...
  try {
    await axios.delete(`${BASE_URL}/rest/gallery/delete/${currentProductDetailGalleryID}`);
...
useEffect(() => {
  console.log("ProductDetailWasCreated đã thay đổi: ", productDetailWasCreated);
  ...
  return (
    ...
    <h5 className="text-danger text-center">productId: {currentProduct?.productId}- {currentProduct?.name}</h5>
        <Modal.Body>
              ...
              <div>
                <div className="container mt-3">
                  <div className="d-flex flex-wrap align-items-center">
                    ...
                    {currentProduct?.productId && productDetails && productDetails.length > 0 ? ( // Kiểm tra nếu có sản phẩm và danh sách màu không rỗng
                          ...
                          <Button
                            ...
                            className="btn-sm position-absolute button-delete-color"
                             onClick={(event) => {
                                ...
                                handleDeleteProductDetail(detailDemo.productDetailId);
                              }}
                          >
            ...
            selectedProductDetail && ( // selectedProductDetail chứa dữ liệu truyền vào
              <Form>
                ...
                <h5 className="text-danger text-center">
                  Id Detail Sản phẩm: {selectedProductDetail?.productDetailId || "Chưa có"} -  {currentProduct?.name || "chưa có"} - <span style={{"backgroundColor": "{selectedProductDetail.color}"}}>màu: {selectedProductDetail.color || "chưa có"}</span>
                </h5>
                <Row>
                  <Col>
                    ...
                    <Form.Group className="mb-1">
                      <Form.Floating>
                        <Form.Control
                          ...
                          name="color"
                          value={
                            selectedProductDetail ? selectedProductDetail.color: ""
                          }
                          onChange={(e) =>
                            handleSelectedColorCombobox(e.target.value)
                          }
                        >
                          {currentProduct?.productId && productDetails && productDetails.length > 0 &&
                            productDetails.map((detailDemo) => (
                              <option
                                key={detailDemo.productDetailId}
                  ...
                  <Form.Group className="mt-3 me-2">
                        <InputGroup>
                          ...
                          {selectedProductDetail.color ? (
                            ...
                            onClick={() => handleOpenModalAddColor(selectedProductDetail?.color )}>                          
                            Sửa màu nè
                            </Button>
                          ): (
                            <>
                    ...
                    <Col className="">
                      <Form.Group className="mt-3 me-2">
                        ...
                        {newColor ? (
                          ...
                          <p className="text-danger mt-2">Vui lòng chọn màu nè</p>
                        )} 
                      </Form.Group>
                    </Col>
                  </Col>
                ...
                <Form.Group controlId="formGallery">
                  ...
                  <div className="btn btn-primary" 
                    onClick={() => 
                      selectedProductDetail?.productDetailId 
                        ? handleUpdateProductDetail(selectedProductDetail?.productDetailId) 
                        : handleAddNewProductDetail(currentProduct!.productId ??  ProductWasCreated )
                    }
                  >
                      {selectedProductDetail?.productDetailId ? "Sửa sản phẩm chi tiết" : "Thêm sản phẩm chi tiết"} 
                    </div>
                  </div>
             
                  {/* Hiển thị bảng gallery */}
                  {/* <Table striped bordered hover className="mt-3">
                    ...
                    <tbody>
                      {selectedGalleryFiles.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan="2">Chưa có gallery</td>
                        </tr>
                      )}
                    </tbody>
                  </Table> */}
                ...
                <Form.Group controlId="formGallery">
                <div className="mb-3">
                  <label className="form-label fw-bold">Gallery của sản phẩm:</label>
                  <div className="d-flex flex-wrap align-items-center">
                    {galleries.length > 0 ? (
                      galleries.map((gallery) => (
                        <div key={gallery.galleryId} className="position-relative me-3 mb-3">
                          <Image 
                          ...
                          <OverlayTrigger overlay={<Tooltip>Xóa</Tooltip>}>
                            <Button
                              ...
                              onClick={(event) => {
                                event.stopPropagation(); // Ngăn sự kiện chọn màu chạy
                                handleDeleteProductDetailGallery(gallery.galleryId);
                              }}
                            >
                ...
                {/* Bảng Kích cỡ */}
                {productDetailWasCreated || selectedProductDetail.productDetailId && (
                ...
                <Form.Label>Kích cỡ:</Form.Label>
                  ...
                  >
                    ...
                    <span>ProductDetail Id from backend {productDetailWasCreated}</span>
                  {isShowAddNewSize && (
                  ...
                  <Table striped bordered hover>
                    ...
                    <tbody>
                      {selectedProductDetail?.productDetailSizes?.length > 0 ? (
                        selectedProductDetail.productDetailSizes.map(
                          (sizeDetail) => (
                            <tr key={sizeDetail.productDetailSizeId}>
                              ...
                              <td>{sizeDetail.size?.sizeName || "Không có kích cỡ"}</td>
                              ...
                              <td className="text-center align-middle">
                                ...
                                >
                                  <Button
                                    variant="danger"
                                    className="m-1"
                                    onClick={() => deleteProductDetailSize(sizeDetail.productDetailSizeId)}
                                  >
                        ...
                        <tr>
                          <td colSpan="5">Chưa có kích cỡ</td>
                        </tr>
                      )}
                    </tbody>
                  </Table>
