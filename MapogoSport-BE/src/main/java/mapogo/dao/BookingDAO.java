...
public interface BookingDAO extends JpaRepository<Booking, Integer> {
	...
	List<Object[]> findRevenueBySportFieldDetailIds(@Param("sportFieldDetailIds") List<Integer> sportFieldDetailIds, @Param("bookingId") List<Integer> bookingId, @Param("status") List<String> status);
	...
	List<BookingDetail> findBookingDetailBySportFieldId(@Param("sportFieldDetailIds") Integer sportFieldDetailIds, @Param("bookingId") List<Integer> bookingId, @Param("status") List<String> status);
...
//	Data chart customer
	...
	@Query("select count(b), b.user.username, b.fullName from Booking b " + "where b.owner.ownerId = :ownerId And b.user.username='sportoffline' "
...
//		       "order by count(b) desc")
	@Query("SELECT COUNT(b) AS totalBookings, b.user.username AS username, u.fullname AS fullname " +
		       ...
		       "WHERE b.owner.ownerId = :ownerId AND b.user.username <> 'sportoffline' " +
		       "GROUP BY b.user.username, u.fullname " +
		       "ORDER BY COUNT(b) DESC")
