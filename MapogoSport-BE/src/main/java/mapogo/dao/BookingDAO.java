...
public interface BookingDAO extends JpaRepository<Booking, Integer> {
	...
	@Query("Select o from  BookingDetail o " + "WHERE o.sportFieldDetail.sportFielDetailId IN :sportFieldDetailIds "
			+ "AND o.booking.owner.ownerId = :ownerId " + "AND CAST(o.date AS DATE) BETWEEN :startDate AND :endDate")
	List<BookingDetail> findBookingDetailByDate(@Param("sportFieldDetailIds") List<Integer> sportFieldDetailIds,
			@Param("ownerId") Integer ownerId, @Param("startDate") Date startDate, @Param("endDate") Date endDate);
	...
	List<Object[]> findRevenueBySportFieldDetailIdsByDate(
			@Param("sportFieldDetailIds") List<Integer> sportFieldDetailIds, @Param("startDate") Date startDate,
...
//	Data chart customer
	...
	@Query("select count(b), b.user.username, b.user.fullname from Booking b " + "where b.owner.ownerId = :ownerId "
			+ "group by b.user.username, b.user.fullname " + "order by count(b) desc ")
